 ...
 * - SPI mode = 1 --> CPOL = 0 && CPHA = 1 (CPHA of the real ADS114S08 is 0, NOT 1)
 */
...
// int tictime;       // uncomment this to enable delay timing
// int delaytime;     // uncomment this to enable delay timing
  ...
  else if (command == SPI_NOP)
  {
    if (data_ready)
    {
       ...
       */
       ...
       //delaytime = micros() - tictime;    // uncomment this to enable delay timing
       //Serial.println(delaytime);         // uncomment this to enable delay timing
       data_ready = false;
    }
  }
...
void loop (void)
{
  if (started)
  {
    ...
    data_ready = true;
    digitalWrite(DRDY, LOW);
    // tictime = micros();      // uncomment this to enable delay timing
  }
